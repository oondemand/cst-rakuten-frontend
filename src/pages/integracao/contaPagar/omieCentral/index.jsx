...
export const IntegracaoContaPagarOmieCentralEsteira = () => {
  const [searchTerm, setSearchTerm] = useStateWithStorage(
    "integracao_conta_pagar_omie_central_searchTerm"
  );
  const [time, setTime] = useStateWithStorage(
    "integracao_conta_pagar_omie_central_time",
    1
  );
  ...
  const {
    ...
    queryFn: async () =>
      await IntegracaoContaPagarOmieCentralService.listarTodos({ time }),
      ...
      <Flex pb="4" justifyContent="space-between">
        <Flex alignItems="flex-end" gap="2">
          ...
          >
            <AsyncButton
              onClick={async () =>
                await IntegracaoContaPagarOmieCentralService.processar()
              }
            />
          </Tooltip>
          <Tooltip
