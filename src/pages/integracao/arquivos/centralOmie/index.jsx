...
export const IntegracaoArquivosCentralOmieEsteira = () => {
  const [searchTerm, setSearchTerm] = useStateWithStorage(
    "integracao_arquivos_central_omie_searchTerm"
  );
  const [time, setTime] = useStateWithStorage(
    "integracao_arquivos_central_omie_time",
    1
  );
  ...
  const {
    ...
    queryKey: ["listar-tickets-integracao-central-omie-arquivos", { time }],
    queryFn: async () =>
      await IntegracaoArquivosCentralOmieService.listarTodos({ time }),
      ...
      <Flex pb="4" justifyContent="space-between">
        <Flex alignItems="flex-end" gap="2">
          ...
          >
            <AsyncButton
              onClick={async () =>
                await IntegracaoArquivosCentralOmieService.processar()
              }
            />
          </Tooltip>
          <Tooltip
