...
export const IntegracaoArquivosCentralOmieDatagrid = () => {
  const { filters, resetFilters, setFilters } = useFilters({
    key: "INTEGRACAO-ARQUIVOS-CENTRAL-OMIE",
  });
  ...
  const { data, error, isLoading, isFetching } = useQuery({
    queryKey: ["listar-integracao-arquivos-central-omie", { filters }],
    queryFn: async () =>
      await IntegracaoArquivosCentralOmieService.listarComPaginacao({
        filtros: filters,
      }),
    ...
    >
      <Box>
        <Text fontSize="lg" color="gray.700" fontWeight="semibold">
          Integrações anexos central {"->"} omie
        </Text>
