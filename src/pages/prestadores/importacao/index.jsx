...
export const ImportPrestadoresPage = () => {
  const [open, setOpen] = useState([0]);

  const { mutateAsync: importPrestadoresMutation, isPending } = useMutation({
    mutationFn: async ({ files }) =>
      await PrestadorService.importarPrestadores({ files }),
    onSuccess() {
      queryClient.refetchQueries(["listar-prestadores"]);
      queryClient.invalidateQueries(["list-prestadores-importados"]);
      toaster.create({
        ...
        <Box>
          <FileUploadRoot
            ...
            onFileAccept={async (e) => {
              await importPrestadoresMutation({ files: e.files });
            }}
            maxFiles={1}
          >
            <FileUploadTrigger>
